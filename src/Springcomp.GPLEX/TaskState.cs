...
namespace QUT.Gplex.Automaton
{
	...
	internal class TaskState : IDisposable
	{
		...
		TextReader FrameReader()
		{
			...
			{
				string gplexxFrame = ResourceUtils.GetContent("gplexx.frame");
		...
		void CopyBufferCode()
		{
			using (StreamWriter writer = new StreamWriter(BufferCodeFile()))
			{
				writer.WriteLine(ResourceUtils.GetContent("ResourceHeader.txt"));
				...
				writer.WriteLine(ResourceUtils.GetContent("GplexBuffers.txt"));
		...
		internal void Process(string fileArg)
		{
			...
			if (inputFile != null)
			{
				...
				try
				{
					...
					scanner = new Lexer.Scanner(inputFile);
						...
						{	// convert to DFSA
							...
							{	// minimize automaton
								...
								{   // emit the scanner to output file
									using (var frameRdr = FrameReader())
									{
										using (var outputWrtr = OutputWriter())
										{
